...
//			20080506 - upped hours awake penalty to after 8 hours, not 4 (lines 98, 303-305)
...
function RoomCheck() { //Combine "FallThrough", "Drowning", and various time checks from "ViewRefresh"
	...
	if ((gametime - lastrest) > 479) { //was 249
...
function ViewRefresh() {// Refresh View
	...
	atcolor = "g";
	if (awaketime > 4) atcolor = "a";
	if (awaketime > 10) atcolor = "r";
	if (awaketime > 16) {
	...
	if (mon[xx][yy][zz] > 0) { //Netscape
		...
		var temp = "images/monsters/"+mon[xx][yy][zz]+"-"+monimg[xx][yy][zz];
		if (hp[xx][yy][zz] < MON[2][mon[xx][yy][zz]]/2) temp = temp + "w";
		var temp = temp + ".gif";
		document.images[imo].src=temp;
	}
	if (mon[xx][yy][zz] == 0) {
